...
-- Two structural fixes to the Diskriminierung lesson body:
--   1. The "Ein Mangel an Inklusion …"-paragraph (content[9]) is now
--      a `think`-variant callout (signal-blue box with a lightbulb
--      icon), matching the LearnWorlds reference.
--   2. The two "Frage Dich selbst:" rose callouts now mark the
--      question paragraph below the bold header with weight: "normal"
--      so only the prompt header is bold.
--
-- Renderer support added in src/lib/lms/renderer.tsx for callout
-- variant "think" and paragraph attrs.weight = "normal" inside a
-- callout.
--
-- Idempotent: full body replacement; re-running sets the same value.
  ...
  "content": [
    ...
    },
    {
      ...
      "content": [
        ...
        { "type": "paragraph", "attrs": { "weight": "normal" }, "content": [{ "type": "text", "text": "Welche Gedanken kommen Dir sofort in den Kopf? Beurteile ich Menschen auf den ersten Blick nach ihrem Aussehen, ihrer Kleidung, ihrem Geschlecht oder ihrer Hautfarbe?" }] }
      ]
    },
    {
      "type": "paragraph",
    ...
    },
    {
      ...
      "content": [
        { "type": "paragraph", "content": [{ "type": "text", "text": "Frage Dich selbst:" }] },
        { "type": "paragraph", "attrs": { "weight": "normal" }, "content": [{ "type": "text", "text": "Erwarte ich von dieser Person etwas anderes, als ich es von jemandem erwarten würde, der mir ähnlicher ist? Behandle ich diese Person anders, wenn sie eine andere ethnische Herkunft, Religion, Geschlechtsidentität oder sexuelle Orientierung hat als ich? Wenn ja, wie?" }] }
      ]
    },
    {
      "type": "paragraph",
