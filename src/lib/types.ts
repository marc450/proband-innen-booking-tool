...
export interface Auszubildende {
  ...
  // Set by /api/auth/set-password-with-token when the customer sets their
  // password via the emailed token link. NULL means no login account
  // exists yet — admin password tooling can show "noch nicht aktiviert"
  // and disable actions.
  user_id: string | null;
...
export interface MerchProduct {
  ...
  /** Primary / hero image. */
  image_url: string | null;
  /** Optional additional images (up to 5 more, migrations 037 + 054).
   *  Rendered as thumbnails on the product detail page that swap the
   *  hero on click. */
